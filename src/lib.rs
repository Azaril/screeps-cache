...
pub trait FastCacheAccessor<'a, T, R>: FastCacheExpiration<T> + FastCacheGet<'a, T, R>
where
    Self: Sized,
{
    fn access<X, F>(self, expiration: X, filler: F) -> CacheAccesor<'a, T, Self, X, F, R>
    where
        F: FnOnce() -> T,
        X: FnOnce(&T) -> bool;
}

pub trait FastCacheMaybeAccessor<'a, T, R>:
    FastCacheExpiration<T> + FastCacheMaybeGet<'a, T, R>
where
    Self: Sized,
{
    fn maybe_access<X, F>(
        ...
        expiration: X,
        filler: F,
    ) -> MaybeCacheAccesor<'a, T, Self, X, F, R>
    where
        F: FnOnce() -> Option<T>,
        X: FnOnce(&T) -> bool;
}

impl<'a, C, T, R> FastCacheAccessor<'a, T, R> for C
where
    C: FastCacheExpiration<T> + FastCacheGet<'a, T, R>,
{
    fn access<X, F>(self, expiration: X, filler: F) -> CacheAccesor<'a, T, Self, X, F, R>
    where
        F: FnOnce() -> T,
...
impl<'a, C, T, R> FastCacheMaybeAccessor<'a, T, R> for C
where
    C: FastCacheExpiration<T> + FastCacheMaybeGet<'a, T, R>,
{
    fn maybe_access<X, F>(self, expiration: X, filler: F) -> MaybeCacheAccesor<'a, T, Self, X, F, R>
    where
        F: FnOnce() -> Option<T>,
        X: FnOnce(&T) -> bool,
    {
        MaybeCacheAccesor {
            state: MaybeCacheState::Unknown(
                MaybeCacheStateUnknown {
                    cache: self,
                    expiration,
                    fill: filler,
...
pub struct CacheAccesor<'c, T, C, X, F, R>
where
    F: FnOnce() -> T,
    X: FnOnce(&T) -> bool,
    C: FastCacheGet<'c, T, R> + FastCacheExpiration<T>,
{
    state: CacheState<'c, T, C, X, F, R>,
    phantom: PhantomData<R>,
}

impl<'c, T, C, X, F, R> Get<R> for CacheAccesor<'c, T, C, X, F, R>
where
    F: FnOnce() -> T,
    X: FnOnce(&T) -> bool,
    C: FastCacheGet<'c, T, R> + FastCacheExpiration<T>,
...
pub enum CacheState<'c, T, C, X, F, R>
where
    F: FnOnce() -> T,
    X: FnOnce(&T) -> bool,
    C: FastCacheGet<'c, T, R> + FastCacheExpiration<T>,
...
impl<'c, T, C, X, F, R> CacheState<'c, T, C, X, F, R>
where
    F: FnOnce() -> T,
    X: FnOnce(&T) -> bool,
    C: FastCacheGet<'c, T, R> + FastCacheExpiration<T>,
    ...
    pub fn into_known(self) -> Self {
        match self {
            CacheState::Unknown(state, _) => {
                ...
                let new_state = CacheStateKnown { data: ref_val };
...
pub struct CacheStateUnknown<'c, T, C, X, F, R>
where
    X: FnOnce(&T) -> bool,
    F: FnOnce() -> T,
    C: FastCacheGet<'c, T, R> + FastCacheExpiration<T>,
...
pub struct MaybeCacheAccesor<'c, T, C, X, F, R>
where
    F: FnOnce() -> Option<T>,
    X: FnOnce(&T) -> bool,
    C: FastCacheMaybeGet<'c, T, R> + FastCacheExpiration<T>,
{
    state: MaybeCacheState<'c, T, C, X, F, R>,
    phantom: PhantomData<R>,
}

impl<'c, T, C, X, F, R> MaybeGet<R> for MaybeCacheAccesor<'c, T, C, X, F, R>
where
    F: FnOnce() -> Option<T>,
    X: FnOnce(&T) -> bool,
    C: FastCacheMaybeGet<'c, T, R> + FastCacheExpiration<T>,
...
pub enum MaybeCacheState<'c, T, C, X, F, R>
where
    F: FnOnce() -> Option<T>,
    X: FnOnce(&T) -> bool,
    C: FastCacheMaybeGet<'c, T, R> + FastCacheExpiration<T>,
...
impl<'c, T, C, X, F, R> MaybeCacheState<'c, T, C, X, F, R>
where
    F: FnOnce() -> Option<T>,
    X: FnOnce(&T) -> bool,
    C: FastCacheMaybeGet<'c, T, R> + FastCacheExpiration<T>,
    ...
    pub fn into_known(self) -> Self {
        match self {
            MaybeCacheState::Unknown(state, _) => {
                ...
                let new_state = MaybeCacheStateKnown { data: ref_val };
...
pub struct MaybeCacheStateUnknown<'c, T, C, X, F, R>
where
    X: FnOnce(&T) -> bool,
    F: FnOnce() -> Option<T>,
    C: FastCacheMaybeGet<'c, T, R> + FastCacheExpiration<T>,
